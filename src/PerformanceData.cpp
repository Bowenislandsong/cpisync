...
void PerformanceData::setsofcontent(GenSync::SyncProtocol setReconProto, vector<int> edit_distRange,
    ...
    if (mode == 1) {
        ...
        catag[1] = "Space";
        plot.create("Sets of Content " + protoName + " " + str_type + "Str " + to_string(str_sizeRange.front()) + "ED" +
                    to_string(edit_distRange.front()) + "TS" + to_string(instance), catag);
    } else if (mode == 4) {
        catag[0] = "Str Size";
        catag[1] = "lvl";
...
//                            initResources(initRes);
                                    if (mode == 1) {
                                        if (levelRange.size() != str_sizeRange.size())
                                    ...
                                    auto r_res = getRsyncStats("Alice" + to_string(instance) + ".txt", "Bob" + to_string(instance) + ".txt");
